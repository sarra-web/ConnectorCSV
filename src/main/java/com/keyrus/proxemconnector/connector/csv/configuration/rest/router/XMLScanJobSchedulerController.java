...
import com.keyrus.proxemconnector.connector.csv.configuration.dto.quartzDto.ScheduleDTORequestXML;
...
import com.keyrus.proxemconnector.connector.csv.configuration.service.xml.ScanJobXML;
...
@RequestMapping("/ScheduleScanXML")
public class XMLScanJobSchedulerController {
    private static final Logger logger = LoggerFactory.getLogger(XMLScanJobSchedulerController.class);
    ...
    @PostMapping()
    public ResponseEntity<ScheduleDTOResponse> scheduleScanXML(@Valid @RequestBody ScheduleDTORequestXML scheduleDTORequest) {
        try {
            ...
            if(dateTime.isBefore(ZonedDateTime.now())) {
                ...
                Logging.putInCSV(LocalDateTime.now().toString(),"/ScheduleScanXML","POST",res.getStatusCode().toString(),"dateTime must be after current time",scheduleDTORequest.getConnectorDAO().userName());
                return res;
            }
            ...
            Logging.putInCSV(LocalDateTime.now().toString(),"/ScheduleScanXML","POST",res.getStatusCode().toString(),"PushToProxem Scheduled Successfully!",scheduleDTORequest.getConnectorDAO().userName());
            return res;
    ...
    private JobDetail buildJobDetail(ScheduleDTORequestXML scheduleDTORequest) {
        ...
        return JobBuilder.newJob(ScanJobXML.class)
